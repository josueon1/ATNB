...
- **Global scope**: `D:/projetos/ATNB/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/projetos/ATNB/.claude/notes/`)
...
2. Glob `D:/projetos/ATNB/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/projetos/ATNB/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/projetos/ATNB/.claude/notes/` — works in any directory
